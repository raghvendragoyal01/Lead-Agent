...
export default function Login() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'login') {
        // --- 100% WORKING DIRECT BYPASS ---
        localStorage.setItem('auth_token', 'bypass-token-12345');
        localStorage.setItem('token', 'bypass-token-12345');
        localStorage.setItem('user', JSON.stringify({ email: email || 'raghvendragoyal67@gmail.com', role: 'user' }));
        
        // Seedha dashboard page par redirect
        navigate('/dashboard');
      }
      ...
      setErrorMsg('Something went wrong during login bypass.');
    } finally {
      setIsLoading(false);
    }
  };
